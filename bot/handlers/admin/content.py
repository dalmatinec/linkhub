...
async def view_categories(ctx: Ctx) -> ViewResult:
    ...
    html = ("🗂 <b>Категории товаров</b>\n\n"
            ...
            "У категории можно указать, как ещё её пишут. Регистр не важен, гласные тоже: достаточно "
            "одного варианта из хаш, хеш, хэш, а hash сам найдёт HSH, sort найдёт SRT. "
            "Латиницу и кириллицу бот не смешивает, поэтому русское написание добавьте один раз сами.\n\n"
...
async def act_category_words(ctx: Ctx, cat_id: str):
    return await ctx.ask("catw", "Как ещё люди могут написать эту категорию? Через запятую, например:\n"
                                 "<code>хэш, гаш, сорт, sort</code>\n"
                                 "Похожие по гласным писать не нужно: хэш сам покроет хаш и хеш.\n\n"
